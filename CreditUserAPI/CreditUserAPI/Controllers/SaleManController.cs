    ...
    public class SaleManController : ApiController
    {
        ...
        public string GetSalemanContribute(dynamic requestData)
        {
            ...
            if (month > 0 && year > 0)
            {
                ...
                string sqlMonth = string.Format("Select COUNT(*) from RegistCode where DATEPART(YY, UseDate) = {2} and DATEPART(MM, UseDate) = {0} and SalemanId = {1} and MemberId <> 0", DateTime.Now.Month, salemanId, DateTime.Now.Year);
