...
//			  console.log(_this.orderlist);
    ...
    chooseAddress(isAddress){//1没有地址 需要添加地址  2有地址 进行选择
    	...
    	}else if(this.$route.query.source=='tastedraw'){
    		this.$router.push({path:'/component/shopaddress',query:{isAddress:isAddress,skudata:this.skudata,source:'tastedraw',orderId:this.orderId}});
    	}
    },
    submitOrder(){//提交订单
    	if(this.addressFlag){//有地址
    		this.btnNum++;
    		if(this.$route.query.source=='tastedraw'&&this.btnNum==1){
    			this.getTime();
...
//				console.log(response.data);
        if(response.data==1&&_this.awardTime!=""){
        	_this.$router.push({path:'/component/submitorder',query:{awardTime:_this.awardTime,SkuName:_this.skudata.SkuName,source:'tastedraw'}});
        }
			})
		},
		getTime(){//获取当前时间
