<template>
	<div class="authenhotel">

		<div class="authenhotelDiv">
			<p class="infor_text">完善此板块信息，您将获得20积分的奖励！</p>
			<x-input title="真实姓名" placeholder="请输入" v-model="username"></x-input>
			<popup-picker title="工作岗位" :data="offerlist" :columns="2" v-model="offer" @on-change="offerShow" cancel-text='取消' confirm-text='确定' value-text-align="left"></popup-picker>
			<!--酒店会员-->
			<div v-show="offerId==1">
				<popup-picker title="酒店区域" :data="addresslist" v-model="hotelarea" @on-change="getCity" :columns="3" show-name cancel-text='取消' confirm-text='确定' value-text-align="left"></popup-picker>
				<x-input title="酒店名称" v-model="hotelName"></x-input>
				<x-input title="酒店地址" placeholder="请输入" v-model="hoteladdress"></x-input>
				<popup-picker title="主营类型" :data="hotelselllist" v-model="hotelSell" value-text-align='left'></popup-picker>
				<popup-picker title="酒店人均消费" :data="hotelSpendList" v-model="hotelSpend" show-name cancel-text='取消' confirm-text='确定' value-text-align='left'></popup-picker>
				<popup-picker title="现在使用欣和产品" :data="IfUseList" v-model="IfUse" show-name cancel-text='取消' confirm-text='确定' value-text-align='left'></popup-picker>
				<popup-picker title="调味品采购权" :data="productSourceList" v-model="productSource" show-name cancel-text='取消' confirm-text='确定' value-text-align='left'></popup-picker>
			</div>
			<!--美食爱好者-->
			<div v-show="offerId==2">
				<popup-picker title="是否有孩子" :data="IfUseList" v-model="HaveChild" show-name cancel-text='取消' confirm-text='确定' value-text-align='left'></popup-picker>
				<popup-picker title="您的家庭收入" :data="HomeIncomeList" v-model="HomeIncome" show-name cancel-text='取消' confirm-text='确定' value-text-align='left'></popup-picker>
				<popup-picker title="您的烹饪频率" :data="CookieNumList" v-model="CookieNum" show-name cancel-text='取消' confirm-text='确定' value-text-align='left'></popup-picker>
			</div>
			<!--调味品供货商-->
			<div v-show="offerId==3">
				<x-input title="所在批发市场名称" v-model="MarketsaleName"></x-input>
				<popup-picker title="门店区域" :data="addresslist" v-model="hotelarea" @on-change="getCity" :columns="3" show-name cancel-text='取消' confirm-text='确定' value-text-align="left"></popup-picker>
				<x-input title="门店名称" v-model="hotelName"></x-input>
				<x-input title="门店地址" placeholder="请输入" v-model="hoteladdress"></x-input>
				<popup-picker title="门店经营范围" :data="MarketScopeList" v-model="MarketScope" show-name cancel-text='取消' confirm-text='确定' value-text-align='left'></popup-picker>
			</div>
...
//				console.log(areaLevel);
				...
				var params={
		  		...
		  		this.$http({
		  			...
		  			data:params
		     	  	...
		     	  	if(_this.isSave==0 && _this.btnNum==1){//没有保存过
		     	  		_this.updateMemberIntegral();//增加20积分
		     	  	}else if(_this.isSave==1){//保存过 
			...
			getMemberProfile(){//获取简历信息 
				...
				this.$http({
					...
					if((data[0].PerConsumption==null||data[0].PerConsumption=="")&&(data[0].IsAnyChild==null||data[0].IsAnyChild=='')&&(data[0].WholesaleName==null||data[0].WholesaleName=='')){
		...
		mounted() {
			...
			$('.authenhotel .weui-cell,.authenhotel button').css({
	  	  		'height':height*0.07
	  	    })
...
<style scoped>
	.authenhotel{overflow-x: hidden;}
    .authenhotelDiv{width: 90%;margin-left: 5%;}
	.infor_text {color: #E83428;font-size: 12px;margin: 15px 0;}
</style>
<style>
	#vux_view_box_body {background: #fff;}
	.authenhotel .weui-btn {margin-top: 40px;background: #E83428;color: white;margin-bottom: 30px;}
	.authenhotel .weui-label { width: 7rem !important;}
	.authenhotel .weui-cells { border: 1px solid #dddddd; border-radius: 0.5rem; }
	...
	.authenhotel .weui-cell{font-size: 13px;color: #3e3e3e;}
	.authenhotel .weui-cell__ft:after{display: none !important;}
	.authenhotel .weui-vcode .weui-icon-clear:before{display: none !important;} 
	.authenhotel .weui-cell{padding: 0 10px!important;border: 1px solid #dddddd; border-radius: 0.5rem;margin-top: 5px;}
	.weui-cell:before{border-top: 0px !important;}
</style>