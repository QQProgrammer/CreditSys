<template>
  <div class="associatechef">
      ...
      <swiper v-model="index" :height="swiperHeight" :show-dots="false" :min-moving-distance = 100000>
        ...
        <swiper-item :key="2">
	        ...
	        <div class="sortsDiv">
	        	...
	        	<span :class="{sortStyle:sortnum==2}" @click="sortClick(2)">编码<img :src="myimg3"></span>
	        </div>
	        <div class="mylist">
    ...
    <div class="mask1" v-show="maskFlag1" ></div><!--遮罩层-->
...
export default{
	...
	methods:{
			tabClick(val){
				...
				if(val==2){
					if(this.selectnum==1||this.selectnum1==1||this.selectnum2==1){
						this.maskFlag1=true;
					}
				}
			},
	  	...
	  	bindMember(){//成为我的 成为该队员名下会员
	  		...
	  		this.$http({
	  			...
	  			if(response.data=="Exc Success"){
		  			...
		  			_this.getMemberByArea(0,2,1,0,0,0);//获取厨师信息 实名认证
		  			_this.getMemberByArea(1,0,1,0,0,0);//获取厨师信息 本区全部会员
		  			_this.getMemberByArea(2,0,1,0,0,0);//获取厨师信息 我的
	  			}
	  		})
	  	},
...
.mask1{width: 100%;height:72%;background: rgba(0,0,0,0.5);position: absolute;top:28%;z-index: 1000;}
